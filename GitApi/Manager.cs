...
namespace GitApi
{
    public class Manager
    {
        ...
        {
            return doRequest<Repo>(string.Format("https://api.github.com/repos/{0}/{1}/commits", repo?.owner.name, repo?.name));
        }

        public static Tree getTree(Commit commit, bool recursive)
        {
            return doRequest<Tree>(commit.commit.tree.url+string.Format("?recursive={0}",recursive));
        }
